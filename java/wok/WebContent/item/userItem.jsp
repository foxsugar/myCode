...
<script type="text/javascript" src="<%=path%>/js/heroData.js"></script>
<script type="text/javascript">
	...
	function chooseMin(){
		...
		function heroInfo(){
			var userHeroId=document.getElementById("userHeroId").value;
			getUserHero(userHeroId,doHeroInfo);
		}
		function doHeroInfo(data){
			for(var i in data.userHeroEquip){
				alert("---------"+i);
			}
		}
</script>
...
<body>
	...
	数量<input id="counts" name="counts">
	<button id="sellItemByAuction" type="submit" >拍卖</button>
	</form>
	</div>
	<div class="style1">
	<form action="<%=path%>/cancelAuctionOperate" method="post">
	auctionId<input id="auctionId" name="auctionId" >
	page<input id="page" name="page" >
	<button id="cancelAuctionOperate" type="submit">取消</button>
	</form>
	</div>
	<div class="style1">
	<button type="button" onclick="javascript:window.location='<%=path%>/getAuctionInfo'"></button>
	<form action="<%=path%>/getItemInfoInAuction" method="post">
	名字<input id="itemName" name="itemName">
	类型<input id="itemType" name="itemType">
	子类型<input id="itemSubType" name="itemSubType">
	品级<input id="itemLevel" name="itemLevel">
	最小等级<input id="levelMin" name="levelMin">
	最大等级<input id="levelMax" name="levelMax">
	<button type="submit">查询</button>
	</form>
	<form action="<%=path%>/getAuctionItemByOrder">
	排序<input id="order" name="order">
	<button type="submit">排序</button>
	</form>
	<form action="<%=path%>/getAuctionItemByPage">
	页数<input id="page" name="page">
	<button type="submit">翻页</button>
	</form>
	</div>
	<div class="style1">
		<form action="<%=path%>/buyItemByAuction">
	物品Id<input id="auctionId" name="auctionId">
	底价<input id="bid" name="bid">
	页数<input id="page" name="page">
	<button type="submit">竞拍</button>
	</form>
	</div>
		<div class="style1">
	<button type="button" onclick="javascript:window.location='<%=path%>/getMyAuctionItemInfo'">获得自己</button>
	<form action="<%=path%>/getMyAuctionItemByPage">
	页数<input id="page" name="page">
	...
	<div class="style1">
	<button type="button" onclick="javascript:window.location='<%=path%>/getMyBidItemInfo'">获得自己竞价</button>
	
			<form action="<%=path%>/buyItemByFixedPriceInBidPage">
	auctionId<input id="auctionId" name="auctionId">
	页<input id="page" name="page">
	<button type="submit">一口价</button>
	</form>
			<form action="<%=path%>/getMyBidItemByPage">
	页<input id="page" name="page">
	<button type="submit">翻</button>
	</form>
	</div>
	<div class="style1">
	<form action="<%=path%>/useItem">
	userItemId<input id="userItemId" name="userItemId">
	counts<input id="count" name="count">
	<button type="submit">使用</button>
	</form>
	</div>
		<div class="style1">
	<form action="<%=path%>/deleteUserItem">
	userItemId<input id="userItemId" name="userItemId">
	<button type="submit">丢弃</button>
	</form>
	</div>
		<div class="style1">
	<form action="<%=path%>/sellItem">
	userItemId<input id="userItemId" name="userItemId">
	<button type="submit">回收</button>
	</form>
	<button onclick="javascript:window.location='<%=path%>/getEquipmentForAuction'">装备</button>
	<button onclick="javascript:window.location='<%=path%>/getMaterialForAuction'">材料</button>
	<button onclick="javascript:window.location='<%=path%>/getBookForAuction'">技能书</button>
	<button onclick="javascript:window.location='<%=path%>/getTavernInfo'">英雄</button>
	<button onclick="javascript:window.location='<%=path%>/refreshHeroList'">刷新</button>
	<button onclick="javascript:window.location='<%=path%>/getAllUserHeroByCharId'">获取自己英雄</button>
	<button onclick="javascript:window.location='<%=path%>/getUserHeroInfo'">获取英雄</button>
	<form action="<%=path%>/employHero">
	位置<input id="num" name="num">
	<button type="submit">雇佣</button>
	</form>
	武将Id<input id="userHeroId" name="userHeroId">
	<button onclick="heroInfo()">查看武将</button>
	<form action="<%=path%>/addHeroExp">
	武将Id<input id="userHeroId" name="userHeroId">
	经验数<input id="addExp" name="addExp">
	<button type="submit">增加经验</button>
	</form>
	<form action="<%=path%>/upHeroLevel">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">升级</button>
	</form>
	<form action="<%=path%>/fireUserHero">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">解雇</button>
	</form>
	<form action="<%=path%>/addPoint">
	武将Id<input id="userHeroId" name="userHeroId">
	武力<input id="forcePointAdd" name="forcePointAdd">
	谋略<input id="intelligencePointAdd" name="intelligencePointAdd">
	耐力<input id="staminaPointAdd" name="staminaPointAdd">
	敏捷<input id="agilityPointAdd" name="agilityPointAdd">
	<button type="submit">加点</button>
	</form>
		<form action="<%=path%>/updateHeroName" method="post">
	武将Id<input id="userHeroId" name="userHeroId">
	改的名字<input id="heroName" name="heroName">
	<button type="submit">改名</button>
	</form>
	<form action="<%=path%>/resetHeroPoint">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">洗髓</button>
	</form>
	<form action="<%=path%>/recoverHp">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">恢复血</button>
	</form>
	<form action="<%=path%>/recoverMp">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">恢复蓝</button>
	</form>
	<form action="<%=path%>/putOnEquipMentForEquip">
	武将Id<input id="userHeroId" name="userHeroId">
	道具Id<input id="userItemId" name="userItemId">
	<button type="submit">换装</button>
	</form>
	<form action="<%=path%>/getOffEquipMentForEquip">
	武将Id<input id="userHeroId" name="userHeroId">
	道具Id<input id="userItemId" name="userItemId">
	<button type="submit">卸装</button>
	</form>
	<form action="<%=path%>/getOffAllEquipMentForEquip">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">全部卸除s</button>
	</form>
	<form action="<%=path%>/getUserEquipByPage">
	页数<input id="page" name="page">
	<button type="submit">翻</button>
	</form>
	<form action="<%=path%>/upHeroGift">
	武将Id<input id="userHeroId" name="userHeroId">
	数量<input id="counts" name="counts">
	<button type="submit">提升</button>
	</form>
	<form action="<%=path%>/getHeroGiftInfo">
	武将Id<input id="userHeroId" name="userHeroId">
	<button type="submit">根骨</button>
	</form>
	</div>
